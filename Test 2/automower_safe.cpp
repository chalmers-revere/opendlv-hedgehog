...
void AutomowerSafe::modeCallback(const std_msgs::UInt16::ConstPtr& msg)
{
    ...
    if (msg->data == 0x90)
    {
        ...
        newControlMainState(AM_STATE_MANUAL);
			pauseMower();
			cutDiscOff();
			setAutoMode();
    }
...
bool AutomowerSafe::update(ros::Duration dt)
{
	...
	if (ros::Time::now() > nextAutomowerInitTime)
        {
		std::cout << "ros::Time::now() > nextAutomowerInitTime" << std::endl;
            serialPortState = AM_SP_STATE_INITIALISING;
            if (initAutomowerBoard())
            {
		std::cout << "initAutomowerBoard() completed" << std::endl;
		...
		std::cout << "initAutomowerBoard() not completed" << std::endl;
